...
export const CLICK_SOUND_LABELS: Record<ClickSound, string> = {
...
export function getClickSound(): ClickSound {
  return (localStorage.getItem(STORAGE_SOUND) as ClickSound) ?? DEFAULT_SOUND
}
...
function getAudioContext(): AudioContext {
  if (!audioCtx) {
    ...
    masterGain.gain.value = getClickVolume()
...
function playSound(type: ClickSound) {
  try {
    ...
    if (type === 'cherry-mx-blue') {
      ...
      noise.connect(g); g.connect(dest())
      noise.start()
      const osc = ctx.createOscillator()
      osc.type = 'triangle'
...
export function previewSound(type: ClickSound) {
  playSound(type)
}
